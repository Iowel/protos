...
type LoginRequest struct {
	...
	Email         string                 `protobuf:"bytes,1,opt,name=email,proto3" json:"email,omitempty"`
	Password      string                 `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"`
	AppId         int32                  `protobuf:"varint,3,opt,name=app_id,json=appId,proto3" json:"app_id,omitempty"` // ID сервиса в который логинится юзер
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type IsAdminRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	UserId        int64                  `protobuf:"varint,1,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"` // ID юзера которого проверяем
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type IsAdminResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	IsAdmin       bool                   `protobuf:"varint,1,opt,name=is_admin,json=isAdmin,proto3" json:"is_admin,omitempty"` // тру / фолс (админ или не админ)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
const file_proto_sso_sso_proto_rawDesc = "" +
	...
	"\bis_admin\x18\x01 \x01(\bR\aisAdmin2\xcf\x01\n" +
	"\x04Auth\x12E\n" +
	"\bRegister\x12\x1b.auth.local.RegisterRequest\x1a\x1c.auth.local.RegisterResponse\x12<\n" +
	"\x05Login\x12\x18.auth.local.LoginRequest\x1a\x19.auth.local.LoginResponse\x12B\n" +
	"\aIsAdmin\x12\x1a.auth.local.IsAdminRequest\x1a\x1b.auth.local.IsAdminResponseB\x0eZ\fprotos.localb\x06proto3"
...
var file_proto_sso_sso_proto_goTypes = []any{
	(*RegisterRequest)(nil),  // 0: auth.local.RegisterRequest
	(*RegisterResponse)(nil), // 1: auth.local.RegisterResponse
	(*LoginRequest)(nil),     // 2: auth.local.LoginRequest
	(*LoginResponse)(nil),    // 3: auth.local.LoginResponse
	(*IsAdminRequest)(nil),   // 4: auth.local.IsAdminRequest
	(*IsAdminResponse)(nil),  // 5: auth.local.IsAdminResponse
}
var file_proto_sso_sso_proto_depIdxs = []int32{
	0, // 0: auth.local.Auth.Register:input_type -> auth.local.RegisterRequest
	2, // 1: auth.local.Auth.Login:input_type -> auth.local.LoginRequest
	4, // 2: auth.local.Auth.IsAdmin:input_type -> auth.local.IsAdminRequest
	1, // 3: auth.local.Auth.Register:output_type -> auth.local.RegisterResponse
	3, // 4: auth.local.Auth.Login:output_type -> auth.local.LoginResponse
	5, // 5: auth.local.Auth.IsAdmin:output_type -> auth.local.IsAdminResponse
